...
Int_t main( Int_t argc, Char_t** argv ){
  ...
  if( !(app.Argc()==5) )
    std::cerr << "Wrong input" << std::endl
	      << "Usage : " << app.Argv(0)
	      << " (char*)infilename (int)obs_ch (int)tp_ch (int)dac" << std::endl
	      << "[e.g]" << std::endl
  ...
  for( Int_t ievt=0; ievt<chain->GetEntries(); ievt++ ){
    ...
    if( fl_message ) printf( "  [Event:%d,Chip:%d,Channel:(obs)%d,(tp)%d] %d entries\n",    t_event,t_chip,obs_ch,tp_ch, (Int_t)hist_1ch->Integral() );
    hist_1ch->SetTitle( Form("Event : %d, Chip : %d, Channel : (obs)%d,(tp)%d, %d entries", t_event,t_chip,obs_ch,tp_ch, (Int_t)hist_1ch->Integral()) );
	...
	hist_1ch_div[idiv]->Draw();
      ...
      can1->cd(1);
      tex1->DrawTextNDC( 0.15, 0.92, Form("Event : %d, Chip : %d, Channel : (obs)%d,(tp)%d, DAC : %d (%d entries)", t_event,t_chip,obs_ch,tp_ch, dac, (Int_t)hist_1ch->Integral()) );
      can1->Update();
      can1->WaitPrimitive();
  ...
  can2->cd(4);
  tex2->DrawTextNDC( 0.20, 0.75, Form("Chip = %d",         t_chip        ) );
